...
def main(root, train_split, weights):
    ...
    while steps < max_steps and epoch < num_epochs:
        ...
        epoch += 1
        # Each epoch has a testing phase
        for phase in ['test']:
            tot_loss = 0.0
            tot_loc_loss = 0.0
            tot_cls_loss = 0.0
            ...
            for i, cls in enumerate(num_classes):
            ...
            if phase == 'test':
                test_score = float(np.trace(confusion_matrix)) / np.sum(confusion_matrix)
                if (test_score > best_test_score or epoch % 2 == 0) and WORLD_RANK == 0:
                    best_val_score = test_score
                    model_name = os.path.join(model_dir, "nslt_" + str(num_classes) + "_" + str(steps).zfill(
                        6) + '_%3f.pt' % test_score)

                    torch.save(ddp_i3d.module.state_dict(), model_name)
                    print(model_name)

                    print(
                        f'TESTING: {phase} Loc Loss: {tot_loc_loss / num_iter:.4f} Cls Loss: {tot_cls_loss / num_iter:.4f} Tot Loss: {(tot_loss * steps_per_update) / num_iter:.4f} Accu :{test_score:.4f}')
...
if __name__ == '__main__':
    ...
    main(root=root, train_split=train_split, weights='/Users/saikiranpennam/Documents/CS-535/Term_project/I3D/nslt_2000_064922_0.100910.pt') #for my reference